...
class NetworkMagmeanR(Network):
    ...
    def make_model(self):
        init_seed = 100
        magr = self.input_networks['magr']
        magr.name = 'magr_model'
        mag = Lambda(globalvars.identity, name=self.model_name+'_lambda_magr')(magr(self.input_layers.values()))
        magmean = Lambda(globalvars.mean, name=self.output_names[1])(mag)
        self.model = Model(inputs=self.input_layers.values(), outputs=[magmean])
        self.model.name = "magmeanr_model"
