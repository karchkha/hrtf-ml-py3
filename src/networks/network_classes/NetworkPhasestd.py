...
class NetworkPhasestd(Network):
    def __init__(self, data=None, inputs=None, input_layers=None, model_details=None, model_details_prev=None, iterations=10, epochs=20, batch_size=32, percent_validation_data=.2): 
        self.custom_objects={'custom_activation_phasestd': NetworkPhasestd.custom_activation_phasestd}
    ...
    def make_model(self):
        ...
        layert = Dense(num_in_neurons, activation=NetworkPhasestd.custom_activation_phasestd, name=self.model_name+'_hidden1')(main_input)
        layert = Dense(int(m.ceil(num_in_neurons/2)), activation=NetworkPhasestd.custom_activation_phasestd, name=self.model_name+'_hidden2')(layert)
        layert = Dense(int(m.ceil(num_in_neurons/4)), activation=NetworkPhasestd.custom_activation_phasestd, name=self.model_name+'_hidden3')(layert)
        layert = Dense(int(m.ceil(num_in_neurons/8)), activation=NetworkPhasestd.custom_activation_phasestd, name=self.model_name+'_hidden4')(layert)
        output = Dense(1, activation=NetworkPhasestd.custom_activation_phasestd, name=self.output_names[0])(layert)
    ...
    @staticmethod
    def custom_activation_phasestd(x):
        return 4*(K.tanh(x/4))
