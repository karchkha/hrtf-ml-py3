...
def make_models(model_names, models_list, created_by = "", run_type = 'train'):
    ...
    if ('real' in model_names) and ('real' not in models_list):
        ...
        nw_real = NetworkReal(OrderedDict([('real',real)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=iterations, epochs=epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, created_by = created_by, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('realmean' in model_names) and ('realmean' not in models_list):
        ...
        input_layers = OrderedDict([('position',pos_input), ('head',head_input), ('ear_left',ear_input_l), ('ear_right',ear_input_r)])
        #Input data to the network, must match the number and names of input layers
        nw_realmean = NetworkRealmean(OrderedDict([('realmean',real)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=iterations, epochs=epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, created_by = created_by, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('realstd' in model_names) and ('realstd' not in models_list):
        ...
        input_layers = OrderedDict([('position', pos_input), ('head', head_input), ('ear_left', ear_input_l), ('ear_right', ear_input_r)])
        nw_realstd = NetworkRealstd(OrderedDict([('realstd',real)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=iterations, epochs=epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, created_by = created_by, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('imag' in model_names) and ('imag' not in models_list):
        ...
        input_layers = OrderedDict([('position',pos_input), ('head',head_input), ('ear_left',ear_input_l), ('ear_right',ear_input_r)])
        nw_imag = NetworkImag(OrderedDict([('imag',imaginary)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=iterations,epochs=epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, created_by = created_by, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('imagmean' in model_names) and ('imagmean' not in models_list):
        ...
        input_layers = OrderedDict([('position', pos_input), ('head', head_input), ('ear_left', ear_input_l), ('ear_right', ear_input_r)])
        nw_imagmean = NetworkImagmean(OrderedDict([('imagmean',imaginary)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=iterations, epochs=epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, created_by = created_by, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('imagstd' in model_names) and ('imagstd' not in models_list):
        ...
        input_layers = OrderedDict([('position', pos_input), ('head', head_input), ('ear_left', ear_input_l), ('ear_right', ear_input_r)])
        nw_imagstd = NetworkImagstd(OrderedDict([('imagstd',imaginary)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=iterations, epochs=epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, created_by = created_by, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('mag' in model_names) and ('mag' not in models_list):
        ...
        input_layers = OrderedDict([('position',pos_input), ('head',head_input), ('ear_left',ear_input_l), ('ear_right',ear_input_r)])
        nw_mag = NetworkMag(OrderedDict([('mag',magnitude)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=iterations,epochs=epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, created_by = created_by, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('magmean' in model_names) and ('magmean' not in models_list):
        ...
        input_layers = OrderedDict([('position', pos_input), ('head', head_input), ('ear_left', ear_input_l), ('ear_right', ear_input_r)])
        nw_magmean = NetworkMagmean(OrderedDict([('magmean',magnitude)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=iterations, epochs=epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('magstd' in model_names) and ('magstd' not in models_list):
        ...
        input_layers = OrderedDict([('position', pos_input), ('head', head_input), ('ear_left', ear_input_l), ('ear_right', ear_input_r)])
        nw_magstd = NetworkMagstd(OrderedDict([('magstd',magnitude)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=iterations, epochs=epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('magraw' in model_names) and ('magraw' not in models_list):
        ...
        input_layers = OrderedDict([('position',pos_input), ('head',head_input), ('ear_left',ear_input_l), ('ear_right',ear_input_r)])
        nw_magraw = NetworkMagraw(OrderedDict([('magraw',magnitude_raw)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=iterations,epochs=epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('magl' in model_names) and ('magl' not in models_list):
        ...
        nw_magl = NetworkMagL(OrderedDict([('magl',magnitude_raw), ('maglmean', magnitude_raw), ('maglstd', magnitude_raw)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=maglr_iterations,epochs=maglr_epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('magmeanl' in model_names) and ('magmeanl' not in models_list):
        ...
        input_layers = OrderedDict([('position',pos_input), ('head',head_input), ('ear_left',ear_input_l)])
        nw_magmeanl = NetworkMagmeanL(OrderedDict([('magmeanl',magnitude)]), input_data, input_layers, input_networks=in_networks, model_details=model_details, model_details_prev=model_details_prev, iterations=maglr_iterations,epochs=maglr_epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('magstdl' in model_names) and ('magstdl' not in models_list):
        ...
        input_layers = OrderedDict([('position',pos_input), ('head',head_input), ('ear_left',ear_input_l)])
        nw_magstdl = NetworkMagstdL(OrderedDict([('magstdl',magnitude)]), input_data, input_layers, input_networks=in_networks, model_details=model_details, model_details_prev=model_details_prev, iterations=maglr_iterations,epochs=maglr_epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('magr' in model_names) and ('magr' not in models_list):
        ...
        nw_magr = NetworkMagR(OrderedDict([('magr',magnitude_raw), ('magrmean', magnitude_raw), ('magrstd', magnitude_raw)]), input_data, input_layers, model_details=model_details, model_details_prev=model_details_prev, iterations=maglr_iterations,epochs=maglr_epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('magmeanr' in model_names) and ('magmeanr' not in models_list):
        ...
        input_layers = OrderedDict([('position',pos_input), ('head',head_input), ('ear_right',ear_input_l)])
        nw_magmeanr = NetworkMagmeanR(OrderedDict([('magmeanr',magnitude)]), input_data, input_layers, input_networks=in_networks, model_details=model_details, model_details_prev=model_details_prev, iterations=maglr_iterations,epochs=maglr_epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, run_type = run_type, batch_size = batch_size, dropout = dropout)
    ...
    if ('magstdr' in model_names) and ('magstdr' not in models_list):
        ...
        input_layers = OrderedDict([('position',pos_input), ('head',head_input), ('ear_right',ear_input_l)])
        nw_magstdr = NetworkMagstdR(OrderedDict([('magstdr',magnitude)]), input_data, input_layers, input_networks=in_networks, model_details=model_details, model_details_prev=model_details_prev, iterations=maglr_iterations,epochs=maglr_epochs, percent_validation_data=percent_valid_points, init_valid_seed=validation_seed, run_type = run_type, batch_size = batch_size, dropout = dropout)
